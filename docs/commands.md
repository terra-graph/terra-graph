...
USAGE
  $ terra-graph diagram [--verbose] [--continueOnError] [--runtimeConfigFile <value>] [--profile <value>]
    [--output <value>...] [--no-plan] [--graph-only] [--terraform-dir <value>] [--plan-file <value>] [--skip-cleanup]
...
FLAGS
  ...
  --output=<value>...          Optional output spec (repeatable):
                               'renderer=<id>;transformers=<t1,t2>;outWriter=<stdout|file>;outFile=<path>'.
...
EXAMPLES
  $ terra-graph diagram --profile my-profile --output "renderer=dot;outWriter=file;outFile=./diagram.dot"

  $ terra-graph diagram --terraform-dir ./infra --profile my-profile --output "renderer=dot;transformers=dotcli;outWriter=file;outFile=./diagram.png" --transformer-dotcli-format png

  $ terra-graph diagram --no-plan --profile my-profile --output "renderer=json;outWriter=file;outFile=./tggraph.json"

  $ terra-graph diagram --runtimeConfigFile ./my-config.yml --output "renderer=dot;outWriter=file;outFile=./diagram.dot"

  $ terra-graph diagram --plan-file ./my-debug.tfplan --skip-cleanup --output "renderer=dot;outWriter=file;outFile=./diagram.dot"
```
...
EXAMPLES
  $ terra-graph diagram:from-dot --dot-input ./graph.dot --profile my-profile --output "renderer=dot;outWriter=file;outFile=./diagram.dot"

  terraform graph | terra-graph diagram:from-dot --profile my-profile --output "renderer=dot;transformers=dotcli;outWriter=file;outFile=./diagram.png" --transformer-dotcli-format png
```
